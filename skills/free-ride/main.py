...
def verify_model_availability(model_id: str, api_key: str) -> bool:
    """Verify that a model is actually available by making a minimal API call.
    ...
    # For hunter-alpha, use the exact format that will be used in config
    if "hunter-alpha" in model_id:
        model_id_for_test = model_id
    else:
        model_id_for_test = model_id

    payload = {
        "model": model_id_for_test,
...
def format_model_for_openclaw(model_id: str, with_provider_prefix: bool = True) -> str:
    ...
    - Primary model: "openrouter/<provider>/<model>" (with provider prefix)
    - Fallbacks/models list: "<provider>/<model>" (without prefix sometimes)
    """
    ...
    # Keep :free suffix as-is from API (some models have it, some don't)
    # Do not add or remove automatically
...
def update_model_config(
    ...
    print(f"Saving config: primary={config['agents']['defaults']['model'].get('primary', 'None')}")
    print(f"Saving config: fallbacks={config['agents']['defaults']['model'].get('fallbacks', [])}")
...
def cmd_auto(args):
    ...
    for m in models:
        if "openrouter/free" not in m["id"]:
            ...
            model_id = m["id"]
            print(f"Checking availability of: {model_id}... ", end="", flush=True)
            if verify_model_availability(model_id, api_key):
                print("✓ Available")
                best_model = m
                break
            else:
    ...
    if not best_model:
        print("\nError: No available models found. Try again later or use 'freeride switch' to manually select a model.")
        sys.exit(1)
