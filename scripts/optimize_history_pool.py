...
# 1. 替换 history_pool 函数
old_func = r'''@app\.route\("/history"\)
def history_pool\(\):
    """显示历史池股票（连续超过指定天数未出现）"""
    ...
    return render_template_string\(HISTORY_TEMPLATE, stale=stale, days=days, latest_date=latest_date\)'''
...
# 2. 更新 HISTORY_TEMPLATE 模板（添加搜索和分页）
